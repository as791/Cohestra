# Scaling Cohestra to ~10,000 Flink jobs

Cohestra models every Flink deployment as a long-lived Temporal *actor workflow*
...
Per-worker (env, surfaced in `deploy/helm/cohestra` `worker.tuning.*`):
...
(`cohestra-leases-<pool>`) with optimistic concurrency. This keeps reservations consistent across
...
Both tiers ship optional HPAs (`deploy/helm/cohestra` `controlApi.autoscaling` /
