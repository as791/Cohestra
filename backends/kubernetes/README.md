...
that the deterministic Cohestra control-plane workflows operate **real** Flink jobs.

It is a separate Go module so the public Cohestra core library keeps no `client-go` dependency.
...
docker build -f backends/kubernetes/Dockerfile -t cohestra-k8s-worker .
```
...
| `FLINK_LEASE_NAMESPACE` | `cohestra-system` | Namespace for capacity-lease ConfigMaps |
