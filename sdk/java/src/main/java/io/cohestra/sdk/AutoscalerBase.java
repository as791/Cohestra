...
/**
 * Base for custom autoscaler implementations that interact with Cohestra.
...
public abstract class AutoscalerBase {
     ...
     */
    protected abstract int evaluate(CohestraResponse actorState);

    /**
     * Run one evaluation cycle: fetch state, evaluate, scale if needed.
     * @return the CohestraResponse from the scale call, or null if no scaling was needed.
     */
    public CohestraResponse tick() {
        CohestraResponse state = deployment.actor();
